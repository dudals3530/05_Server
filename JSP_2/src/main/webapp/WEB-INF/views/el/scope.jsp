...
<h1>Servlet/JSP 범위별 내장 객체</h1>
	...
	<h3>범위별 객체에 세팅된값(속성) 얻어오기</h3>
    <pre>
        - 제출된 파라미터 얻어오기 EL : \${param.key}

        - 범위별 객체에 세팅된 값 얻어오는 EL
        1) \${000Scope.key}  ex) \${requestScope.key}

        2) \${key}
        -> 좁은 범위 객체 부터 탐색하여 일치하는 key가 있으면 얻어옴.
    </pre>

	<%
		//page scope 객체에 값 세팅
		pageContext.setAttribute("pageValue","pageScope 객체에 세팅한값");
	%>

    <p>page scope: ${pageValue}</p> <!-- =결국 pageScope.pageValue-->
