...
<body>
	...
	<c:set var="test2" scope="page" value ="페이지" ></c:set>
	<c:set var="test2" scope="request" value ="리케스트" ></c:set>
	<c:set var="test2" scope="session" value ="세션" ></c:set>
	<c:set var="test2" scope="application" value ="어플리케이션" ></c:set>
	...
	<pre> 
		...
		*주의사항 : else 구문이 없다 !
		...
		- 작성법 2 : 작성된 조건식의 결과는 무조거 T/F
	</pre>
	
	<c:set var="test3" value="금요일" ></c:set>
	...
	<h3>4. 연속된 조건문 - c:choose, c:when , c:otherwise</h3>
	 ...
	 <c:choose>
	 	<%-- 
	 		...
	 		1) null 인경우 true
	 		2) 연산 대상이 배열 /컬렉션 인데 내부에 아무 요소도 없는 경우 true
	 		
	 		나머지 false
	 	
	 		<c:when test="${param.age == nulll}">
   ...
   6) varStatus : 현재 반복 상태와 관련된 정보를 제공하는 변수 선언
	...
	<c:forEach var="i" begin="1" end="6" step="1">
		<h${7-i}>${7-i} 태그입니다.</h${7-i}>
	</c:forEach>
	
	<h3>5-2) 일반 for문 + 컬랙션 List 사용하기</h3>
	
	<ul>
		<li>nameList: ${nameList}</li>
		
		<li>
		<%-- 접두사 fn 이 추가된 EL 구문사용 
			fn(function, 기능) : 문자열 , 컬렉션 관련 기능 제공하는 JSTL-EL
			%{fn:length(컬렉션,배열명,문자열)}
		--%>
