 ...
 * 2.@WevServlet()어노테이션 이용방법
 * 
 * */
  
// "/example1" 이라는 클라이언트의 요청을 받아서 처리한후 응답을 해줄 서블릿 클래스를 만든다.
// 서블릿 클래스 ExampleController1을 만든다.

// Constroller : 요청 (Request)에 따라 어떤 Service를 호출할지 "제어"하고,
//				 어떻게 응답(Response)을 할지 "제어" 하는역할. 
public class ExampleController1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 서버 -> 클라이언트로 연결된 출령용 스트림 제공 (PrintWriter)
		
		System.out.println("---이름, 나이를 입력받아 처리하는 서블릿 코드");
		
		// 요청 시 입력된 이름,나이를 전달받아오기
		
		// Parameter : 매개변수 == 다른곳의 값을 전달받아올때 사용
		
		// req.getParameter("name(키) 속성값") ;
		// -> 요청 시 전달된 데이터 중
